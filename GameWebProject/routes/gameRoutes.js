...
//     res.render("index", { featuredGames, newReleases, trending, page, totalPages });
//   } catch (err) {
//     console.error(err);
...
// // Dynamic route for game descriptions
...
router.post("/game", isAdmin, async (req, res) => {
  ...
  try {
    const newGame = new Game({ name, oldPrice, discountPercentage, descBigImg, category });
    await newGame.save();
